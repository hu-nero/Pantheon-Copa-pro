...
int AdaptiveProCC::flow_id_counter = 0;

double AdaptiveProCC::current_timestamp( void ){
  return cur_tick;
}

void AdaptiveProCC::init() {
  if (num_pkts_acked != 0) {
...
void AdaptiveProCC::update_delta(bool pkt_lost __attribute((unused)), double cur_rtt) {
  double cur_time = current_timestamp();
    ...
    if (prev_delta_update_time == 0. || prev_delta_update_time_loss + cur_rtt < cur_time)
    {
        ...
        else
        {
            if (vRtt.size() >= 10)
            {
                ...
                // wma value
                double smoothed_data = DeltaPlus.wma(vRtt);
                //std::vector<double> smoothed_data = DeltaPlus.wma(vRtt);
                delta = DeltaPlus.adjust_delta(cur_time, queuing_delay, vRtt, smoothed_data, 0.01);
            }
        }
        prev_delta_update_time = cur_time;
    }
...
double AdaptiveProCC::randomize_intersend(double intersend) {
...
void AdaptiveProCC::update_intersend_time() {
  ...
  if (queuing_delay == 0)
    ...
    target_window = (1+delta) * rtt / (queuing_delay * delta);
  ...
  if (slow_start) {
    if (do_slow_start || target_window == numeric_limits<double>::max()) {
      ...
      {
        slow_start = false;
        slow_start_end_time = current_timestamp(); // 记录慢启动结束的时间点
      }
    }
    else {
      assert(false);
  ...
  // Update the window
      ...
      /*
      if ((cur_time - slow_start_end_time >= 0.5) &&
          (cur_time - slow_start_end_time <= 5))
      {
          // 如果还没有过5秒，就保持当前状态，不进行更新
          return;
      }
      */
    ...
    if (update_amt > _the_window * delta / (1+delta)) {
      update_amt /= 2;
    }
    ...
    if (_the_window < target_window) {
      ++ update_dir;
      _the_window += (1+delta) * update_amt / (delta * _the_window);
    }
    else {
      -- update_dir;
      _the_window -= (1+delta) *  update_amt / (delta * _the_window);
...
void AdaptiveProCC::onACK(int ack, 
  ...
  if (!slow_start)
  {
      vRtt.push_back(cur_time - sent_time);
      if (vRtt.size() > 10)
      {
          vRtt.pop_front();
      }
...
void AdaptiveProCC::onPktSent(int seq_num) {
  double cur_time = current_timestamp();
...
void AdaptiveProCC::close() {
}

void AdaptiveProCC::onDupACK() {
  ///num_pkts_lost ++;
...
void AdaptiveProCC::interpret_config_str(string config) {
